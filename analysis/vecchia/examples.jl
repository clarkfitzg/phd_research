...
# Simple case: correlation decays exponentially with distance
...
for r in rhotest
    ...
    push!(llv_100, vecchia_elementwise(data, Sigma_r, 100))
end
...
plot(rhotest - rhotrue, llv_default - maximum(llv_default)
     , label = "Vecchia's with default number of neighbors")
plot(rhotest - rhotrue, llv_100 - maximum(llv_100)
     , label = "Vecchia's with 100 neighbors")
